...
interface FileUploadProps {
    /** Firebase Storage instance from the consuming app's client. */
    storage: FirebaseStorage;
    label: string;
    /** Storage path prefix; the file name + timestamp is appended. */
    path: string;
...
/**
 * App-agnostic file upload control. The Firebase Storage instance is
 * passed in so the same component works for admin and the web teacher
 * portal.
 */
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 bg-gray-50 flex flex-col items-center justify-center text-center transition-colors hover:bg-gray-100">
                ...
                {error && <p className="mt-2 text-sm text-red-600 font-medium">Error: {error}</p>}
            </div>
        </div>
    );
}
