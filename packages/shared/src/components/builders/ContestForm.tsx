...
export interface ContestFormProps {
    contest?: Contest;
    /** User ID for createdBy field. */
    actingUserId: string;
    /** Firebase Storage for RichTextEditor uploads. */
    storage: FirebaseStorage;
    /** Called on form submit with the payload and a callback to redirect. */
    onSubmit: (payload: Record<string, unknown>, onSuccess: () => void) => Promise<void>;
    /** Data loaders for dropdowns. */
    loadTestSeries: () => Promise<TestSeries[]>;
    loadTestsInSeries: (seriesId: string) => Promise<Test[]>;
    loadQuizzes: () => Promise<Quiz[]>;
    /** Optional: question bank picker component. */
    QuestionBankPicker?: React.ComponentType<{
        open: boolean;
        mode: "quiz";
        onClose: () => void;
    ...
    /** Optional: markdown parser for custom questions. */
    parseMarkdown?: (md: string) => { questions: CreateQuizQuestionInput[]; errors: { line: number; message: string }[] };
    /** Optional: markdown template text. */
...
export function ContestForm({ contest, actingUserId, storage, onSubmit, loadTestSeries, loadTestsInSeries, loadQuizzes, QuestionBankPicker, parseMarkdown, markdownTemplate, onBankQuestionsUsed, aiToolbarSlot, mode = "admin" }: ContestFormProps) {
    ...
    useEffect(() => {
        ...
        async function loadTests() {
            try {
                ...
                const items = await loadTestsInSeries(form.seriesId);
                setTests(items);
                setForm((current) => {
    ...
    const handleAddCustomQuestionsFromBank = async (bankQuestions: QuestionBankQuestion[]) => {
        const mapBankType = (t: QuestionBankQuestion["type"]): Exclude<QuestionType, "code"> | null => {
            if (t === "code" || t === "coding") return null;
            if (t === "mcq" || t === "msq" || t === "true_false" || t === "aptitude") return "mcq";
            return "text_input";
        };
        const quizCompatibleQuestions = bankQuestions
            .map((q) => ({ q, mappedType: mapBankType(q.type) }))
            .filter((item): item is { q: QuestionBankQuestion; mappedType: Exclude<QuestionType, "code"> } => item.mappedType !== null);
        if (quizCompatibleQuestions.length === 0) return;

        setCustomQuestions((current) => [
            ...current,
            ...quizCompatibleQuestions.map(({ q, mappedType }): CustomQuestionDraft => ({
                type: mappedType,
        ...
        if (onBankQuestionsUsed) await onBankQuestionsUsed(quizCompatibleQuestions.map(({ q }) => q.id));
    };
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        const customParse = form.sourceType === "custom" && customInputMode === "markdown" && form.customQuestionsMarkdown.trim() && parseMarkdown
            ? parseMarkdown(form.customQuestionsMarkdown)
            : { questions: [], errors: [] };
        ...
        try {
            ...
            setError(null);
            if (contest) {
                await onSubmit({ id: contest.id, ...payload }, () => router.push("/contests"));
            } else {
                await onSubmit(payload, () => router.push("/contests"));
            }
        } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {isTeacherMode && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Contest will be saved as <strong>private</strong> to your classroom. Submit it for review
                                ...
                                {customInputMode === "markdown" && (
                                    <>
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            size="sm"
                                            onClick={() => updateField("customQuestionsMarkdown", markdownTemplate || "")}
                                        >
                                            Insert Template
                                        </Button>
                        ...
                        <div className="space-y-5 p-6">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <button
                                    ...
                                    className="rounded-full border border-slate-200 bg-white px-3 py-1 text-sm font-bold text-slate-500 hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900"
                                >
                                    Close
                                </button>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    ] as { value: Exclude<QuestionType, "code">; label: string }[]).map((option) => (
                                        <button
                                            key={option.value}
                                            type="button"
                                            ...
                                            className={`rounded-lg border px-3 py-1.5 text-sm font-semibold transition ${
                                                editingCustomQuestion.type === option.value
