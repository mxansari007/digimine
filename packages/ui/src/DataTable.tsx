...
function getPageItems(page: number, pageCount: number): Array<number | "ellipsis-left" | "ellipsis-right"> {
    if (pageCount <= 7) {
...
export function DataTable<T>({
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-white/70 bg-white/90 shadow-[0_18px_50px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/70 backdrop-blur-sm">
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-100 text-left">
                    <thead className="bg-gradient-to-r from-slate-50 via-white to-primary-50/40">
                        <tr>
                            {columns.map((column) => (
                                <th
                                    key={column.key}
                    ...
                    <tbody className="divide-y divide-slate-100/80 bg-white/90">
                        {isLoading ? (
                            ...
                            data.map((row, index) => (
                                ...
                                >
                                    {columns.map((column) => (
                                        <td key={column.key} className={`px-5 py-4 align-middle text-sm text-slate-600 ${column.className || ""}`}>
            ...
            {footer && <div className="border-t border-slate-100/80 bg-gradient-to-r from-slate-50/95 to-primary-50/40 px-4 py-3">{footer}</div>}
        </div>
    );
}
                ...
                {onPageSizeChange && (
                    <label className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <select
                            ...
                            className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm font-medium text-slate-700 outline-none transition-colors focus:border-primary-400 focus:ring-2 focus:ring-primary-100"
                        >
                        ...
                        typeof item === "number" ? (
                            <button
                                ...
                                className={`h-9 min-w-9 rounded-lg px-3 text-sm font-semibold transition-colors ${
                                    item === safePage
                                        ? "bg-gradient-to-b from-primary-500 to-primary-600 text-white shadow-sm shadow-primary-500/20"
                            ...
                            <span key={item} className="px-2 text-sm text-slate-400">...</span>
                        )
                    ))}
                    <Button
                        type="button"
