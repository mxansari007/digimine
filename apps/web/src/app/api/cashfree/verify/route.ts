...
export async function POST(request: NextRequest) {
    try {
        ...
        if (newStatus === "completed" && !accessKey) {
            // specific implementation for unique key, simple random string or UUID
            // Using simple random string for readability but can use crypto.randomUUID() if available in env
            // Using Math.random is weak, lets use a simple alphanum string
