...
function teacherQuestionBankCollection(teacherId: string) {
...
function normalizeOptions(
  data: Pick<CreateQuestionBankQuestionInput, "type" | "options">
) {
...
export async function getTeacherQuestionBankQuestions(
  teacherId: string,
  filters?: QuestionBankFilters
): Promise<QuestionBankQuestion[]> {
  const snapshot = await getDocs(
    query(
      teacherQuestionBankCollection(teacherId),
  ...
  return snapshot.docs
    ...
    .filter(
      (question) => filters?.includeCode || !isCodeBankType(question.type)
    )
    .filter(
      (question) =>
        !filters?.type ||
        filters.type === "all" ||
        toQuestionType(question.type) === toQuestionType(filters.type)
    )
    .filter(
      (question) =>
        !filters?.difficulty ||
    ...
    .filter(
      (question) =>
        !filters?.topic ||
        question.topic.toLowerCase() === filters.topic.toLowerCase()
    )
    .filter(
      (question) =>
        !filters?.category ||
...
export async function getTeacherQuestionBankQuestion(
  teacherId: string,
  id: string
): Promise<QuestionBankQuestion | null> {
  const snapshot = await getDoc(
    doc(teacherQuestionBankCollection(teacherId), id)
  );
  if (!snapshot.exists()) return null;
  return mapDoc<QuestionBankQuestion>(snapshot);
}

export async function createTeacherQuestionBankQuestion(
  teacherId: string,
  data: CreateQuestionBankQuestionInput
): Promise<string> {
  const questionId = uuidv4();
  const questionRef = doc(teacherQuestionBankCollection(teacherId), questionId);
  ...
  const payload: Omit<QuestionBankQuestion, "id"> = {
    ...
    correctAnswer: isTextBankType(data.type)
...
export async function updateTeacherQuestionBankQuestion(
  teacherId: string,
  data: UpdateQuestionBankQuestionInput
): Promise<void> {
  const { id, ...updateData } = data;
  const options =
    updateData.type && isChoiceBankType(updateData.type) && updateData.options
      ? updateData.options.map((option) => ({
          ...option,
          id: (option as { id?: string }).id || uuidv4(),
        }))
      : updateData.options;
...
export async function deleteTeacherQuestionBankQuestion(
  teacherId: string,
  id: string
): Promise<void> {
  await deleteDoc(doc(teacherQuestionBankCollection(teacherId), id));
}

export async function incrementTeacherQuestionBankUsage(
  teacherId: string,
  questionIds: string[]
): Promise<void> {
  await Promise.all(
...
export function questionBankToTestQuestionInput(
  ...
  return {
    ...
    supportedLanguages: isCodeBankType(question.type)
    ...
    codeScoringMode: isCodeBankType(question.type)
      ? question.codeScoringMode
      : undefined,
    timeLimit: isCodeBankType(question.type) ? question.timeLimit : undefined,
    memoryLimit: isCodeBankType(question.type)
      ? question.memoryLimit
      : undefined,
  };
}

export function questionBankToQuizQuestionInput(
  question: QuestionBankQuestion,
  quizId: string,
  order: number
): CreateQuizQuestionInput {
  ...
  return {
    quizId,
    ...(baseQuestionPayload(question) as Omit<
      CreateQuizQuestionInput,
      "quizId" | "type"
